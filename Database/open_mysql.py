...
class OperMysql:
    ...
    def __init__(self, host="", port=0, user="", password="", db=""):
        self.host = host
        self.port = port
        self.user = user
    ...
    def connect(self):
        try:
            # autocommit=1 设置自动提交,避免每次执行sql语句都要commit,但是要避免对数据库的修改操作
            self.conn = pymysql.connect(host=self.host, port=self.port, user=self.user, password=self.password, db=self.db,autocommit=1)
